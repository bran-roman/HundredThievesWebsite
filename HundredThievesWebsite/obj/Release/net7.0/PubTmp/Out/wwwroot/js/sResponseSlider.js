...
$(document).ready(function() {
	...
	var contentAmt = $("#container ._content").length;  // Number of slides
	var loop = true;
	...
	$(".slide-left, .slide-right").on("click", function() {
		var slide = parseInt($("#container ._content.active").attr("data-slide"));
	...
	function changeSlide(clicked, method) {
		...
		if (method != "clicked") {
			// Figure slides
			var currentSlide = $("#container ._content.active").attr("data-slide");
			...
			if (nextSlide > contentAmt) {
				...
				if (compact == true) {
					// Transitioney stuff
					$("#container ._content:not(.active)").addClass("transition");
					setTimeout(function() {
						$("#container ._content").removeClass("transition");
					}, transTimer);
				}
			}
		...
		$("#container ._content").removeClass("active");
		$("#container ._content[data-slide='"+nextSlide+"']").addClass("active");
	} // END function
}); // END .ready